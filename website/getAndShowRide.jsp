...
<%
    ...
    String days1 = ("" + request.getParameter("days")).trim();
    String str = AddGroup.getCustGroups(user,"" + request.getParameter("homelat"), "" + request.getParameter("homelng"), "" + request.getParameter("officelat"), "" + request.getParameter("officelng"), days1, request.getParameter("usertime").trim() + ":00");    
    ...
    ResultSet check = st.executeQuery("select * from cust_group where C_id=" +user+" AND G_id = "+ g_id + ";");
    if(check.next()){
        response.sendRedirect("Allreadymember.jsp");
        return;
    }
...
String url = "joinRideFromAll.jsp?homelat=" + homelat1 + "&homelng=" + homelng1 + "&officelat=" + officelat1 + "&officelng=" + officelng1 + "&points=" + i + "&driver="
